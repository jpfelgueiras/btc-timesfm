#!/usr/bin/env python3
"""Unit tests for durable forecast-history integrity audit and repair tooling."""
...
class HistoryAuditTests(unittest.TestCase):
    ...
    def _insert_prediction(
        ...
        *,
        ...
        source = 100.0
        predicted = predicted if predicted is not None else source + horizon
    ...
    def test_healthy_database_has_no_issues_after_outcomes_are_matured(self) -> None:
        self._mature_all()
    ...
    def test_dry_run_reports_missing_matured_outcomes_without_writing(self) -> None:
        target = int((self.origin + timedelta(hours=2)).timestamp())
        report = audit_database(
            self.db,
            now=self.now,
            actual_by_timestamp={target: 101.0},
        )
        issue = next(
            issue
            for issue in report["issues"]
            if issue["code"] == "missing_matured_outcomes"
        )
    ...
    def test_repair_fills_explicit_outcomes_and_is_idempotent(self) -> None:
        actuals = {
            int((self.origin + timedelta(hours=2)).timestamp()): 101.0,
            int((self.origin + timedelta(hours=4)).timestamp()): 104.0,
        }
        ...
        self.assertEqual(first["summary"]["warnings"], 0)
    ...
    def test_repair_recomputes_safe_derived_fields(self) -> None:
        self._mature_all()
        connection = sqlite3.connect(self.db)
        try:
            connection.execute(
                ...
                SET target_at = ?,
                    ...
                    matured_at = NULL
                WHERE origin_at = ? AND model_name = 'ensemble' AND horizon_hours = 2
                """,
                (
        ...
        dry = audit_database(self.db, now=self.now)
        codes = {issue["code"] for issue in dry["issues"]}
        ...
        self.assertEqual(repaired["summary"]["errors"], 0)
        ...
        try:
            row = connection.execute(
                """
                SELECT target_at, predicted_change_pct, absolute_error_pct,
                       direction_correct, matured_at
            ...
            connection.close()
        self.assertEqual(row[0], (self.origin + timedelta(hours=2)).isoformat())
        self.assertAlmostEqual(row[1], 2.0)
        self.assertIsNotNone(row[2])
        self.assertIsNotNone(row[3])
        self.assertIsNotNone(row[4])

    def test_orphan_prediction_is_reported_and_never_deleted(self) -> None:
        connection = sqlite3.connect(self.db)
        try:
    ...
    def test_model_rows_without_ensemble_are_detected(self) -> None:
        ...
        issue = next(
            issue
            for issue in report["issues"]
            if issue["code"] == "orphan_model_groups"
        )
        self.assertEqual(issue["severity"], "error")
        self.assertEqual(issue["count"], 1)

    def test_invalid_required_fields_are_actionable(self) -> None:
        connection = sqlite3.connect(self.db)
        try:
            connection.execute(
                "UPDATE forecast_origins SET market_features_json = 'not-json' "
                "WHERE origin_at = ?",
                (self.origin.isoformat(),),
            )
            connection.commit()
        ...
        issue = next(
            issue
            for issue in report["issues"]
            if issue["code"] == "invalid_origin_fields"
        )
        self.assertEqual(issue["severity"], "error")
        self.assertIn(
            "invalid_market_features_json",
            issue["examples"][0]["problems"],
        )
    ...
    def test_actuals_loader_accepts_iso_and_unix_keys(self) -> None:
        ...
        path.write_text(
            json.dumps({iso: 101.0, str(unix): 104.0}),
            encoding="utf-8",
        )
        loaded = load_actuals(path)
        self.assertEqual(
            loaded[int((self.origin + timedelta(hours=2)).timestamp())],
            101.0,
        )
