...
The ruleset requires changes to use pull requests. Its current required approving review count is zero; mandatory approving reviews are not part of this drift-check contract. Keep the workflow/job names and these contexts synchronized when renaming checks. The explicit mypy file list in `tests.yml` is the staged type-check scope; extending it should be done deliberately as modules become clean.

Maintainers can verify the live repository configuration with `gh auth login` and `python scripts/check_required_checks.py` (or pass `--repo OWNER/REPOSITORY`). The command reads all repository and inherited ruleset pages through the GitHub API, considers only active rules that apply to `OWNER/REPOSITORY` and `refs/heads/main`, and fails if the canonical check, either compatibility context, dependency audit, or a pull-request rule is missing. A pull-request rule satisfies PR enforcement regardless of its `required_approving_review_count`; that count is currently zero and approving reviews are not required by this contract. The authenticated account needs permission to view rulesets. Run this after ruleset edits and periodically; repository ruleset configuration is external to git and cannot be enforced by a workflow when the ruleset itself is removed or weakened.
