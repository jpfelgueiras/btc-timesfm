...
The current schema version is **4**. Schema version 3 adds reproducibility metadata to each forecast origin:
...
The complete manifest is first-write-wins with the forecast origin, so a manual rerun cannot replace the code/configuration/data identity attached to the original historical forecast. CSV and JSONL forecast exports include these fields.

Schema version 4 adds the `drift_events` audit table used by production drift detection. Warning and severe events store the observed evaluation origin, evaluation/run identity, signal key and kind, severity, optional model/horizon or feature identity, and the machine-readable metric payload. The unique `(evaluation_origin_at, signal_key, severity)` key makes repeated evaluation of the same observed state idempotent.
...
`configuration_id` is a stable fingerprint for comparing runs made with the same effective configuration. `experiment_run_id` identifies one execution. Production drift severity, thresholds and adaptive-confidence behavior are included in the forecast run parameters so drift-modified forecasts remain reproducible. See `EXPERIMENTS.md` and `DRIFT_DETECTION.md` for the reproduction procedures.
