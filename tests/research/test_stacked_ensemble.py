...
def sample(index: int, origin: int) -> dict:
    ...
    return {"origin_timestamp": origin, "current_price": current, "actuals": {f"{hour}h": current + 0.15 for hour in HORIZONS}, "forecast": {"latest_close_at": datetime.fromtimestamp(origin, tz=timezone.utc).isoformat(), "latest_close_usd": current, "regime": "range", "market_features": features, "model_predictions": models, "predictions": {}}}
